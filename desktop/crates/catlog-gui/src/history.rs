...
impl HistoryPage {
    ...
    pub fn show(
        ...
        egui::ScrollArea::vertical().show(ui, |ui| {
            ...
            if matches!(def.field_type, FieldType::Number | FieldType::UnitValue) {
                self.show_graph(ui, store, t, entity, def);
            }
    ...
    /// the trend on request, the change per month under it.
    fn show_graph(&mut self, ui: &mut Ui, store: &Catalog, t: &L10n, entity: &str, def: &FieldDef) {
        ...
        if trend && let Some(fit) = trend_line(&points, from, to) {
            ...
            let per_month = match def.field_type {
                FieldType::UnitValue => field_value_display(
                    t,
                    Some(def),
                    Some(&format!("{}", fit.per_month)),
                    self.units,
                ),
                _ => format_number(t.locale(), fit.per_month, 2),
            };
            ui.label(t.trend_per_month(&format!("{sign}{per_month}")));
        }
