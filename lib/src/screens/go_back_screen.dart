...
class _GoBackScreenState extends State<GoBackScreen> {
  ...
  Future<void> _mark() async {
    ...
    widget.store.addSavePoint(cause: SaveCause.manual, label: name);
    setState(() {});
  }

  Future<void> _goBack(SavePoint point) async {
    final done = await confirmAndRevert(context, widget.store, point,
        saveTo: widget.saveTo, body: (t, count) => t.goBackBody(count));
  ...
  Widget build(BuildContext context) {
    final t = context.t;
    final all = savePointsOf(widget.store);
...
IconData _icon(String cause) => switch (cause) {
      SaveCause.import => Icons.download,
      SaveCause.sync => Icons.sync,
      SaveCause.merge => Icons.merge,
      SaveCause.hardDelete => Icons.delete_forever_outlined,
      SaveCause.archive => Icons.inventory_2_outlined,
      _ => Icons.bookmark_outline,
    };

/// A moment as a sentence: what was about to happen.
String momentTitle(AppLocalizations t, SavePoint point) =>
    switch (point.cause) {
      SaveCause.import => t.momentImport,
      SaveCause.sync => t.momentSync,
      SaveCause.merge => t.momentMerge,
      SaveCause.hardDelete => t.momentHardDelete,
      SaveCause.archive => t.momentArchive,
      _ => point.label ?? t.momentManual,
    };

/// What it was about, and when — the label unless it is already the
/// title, then just the date.
String momentDetail(BuildContext context, SavePoint point) {
  final when = DateFormat.yMd(Localizations.localeOf(context).toString())
      .add_Hm()
      .format(point.at.toLocal());
  final label = point.cause == SaveCause.manual ? null : point.label;
